...
-- Example of SELF JOIN (Finding an employee's manager)
...
```sql
-- Grouping by department, filtering before (WHERE) and after (HAVING) grouping
SELECT DepartmentID, SUM(Salary) AS TotalSalary
FROM Employees
WHERE IsActive = 1        -- Filter raw records first
GROUP BY DepartmentID
HAVING SUM(Salary) > 50000; -- Filter the aggregated results
```
...
UNION ALL
SELECT FirstName, LastName FROM ArchivedUsers;
```
